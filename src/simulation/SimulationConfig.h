...
public:
    ...
    SimulationConfig(Simtypes::SIZE bodycount, Simtypes::FLOAT endTime,
                     Simtypes::FLOAT dt, Simtypes::v3 * positions,
    ...
    SimulationConfig(std::string filename) {
        try {
            ...
            if(table->contains_qualified("data.file")) {
                ...
                DEBUGOUTPUT("Using single data file: " << filename
                                                       << std::endl);
                ...
                DEBUGOUTPUT("positions file: " << positions_filename
                                               << std::endl);
                DEBUGOUTPUT("velocities file: " << velocities_filename
                                                << std::endl);
            ...
            std::cerr << "Error during reading data files occured: "
                      << failure.what() << std::endl;
        }

        DEBUGONLY(std::cout << "Data read: " << std::endl; printBodies(););
    }

    void printBodies() {
        std::cout << "MASS\tX\tY\tZ\tVX\tVY\tVZ" << std::endl;
        for(int i = 0; i < bodycount; i++) {
            std::cout << masses[i] << "\t" << positions[i].x << "\t"
...
private:
    // check if all rows contain the expected number of colums, and if enough
    // row for all particles are present
    void checkRows(std::vector<std::vector<std::string>> rows,
                   Simtypes::SIZE bodycount, Simtypes::SIZE expectedColums,
                   std::string dataName) {
        if(rows.size() < bodycount) {
            std::cerr << dataName << " too short, only " << rows.size()
                      << " rows found, expected " << bodycount << std::endl;
            std::abort();
        }

        for(Simtypes::SIZE i = 0; i < bodycount; i++) {
            if(rows[i].size() != expectedColums) {
                std::cerr << dataName << " for body " << bodycount
                          << " invalid!" << std::endl;
                std::abort();
            }
        }
    ...
    void readSplit(std::string masses_filename, std::string positions_filename,
                   std::string velocities_filename) {
