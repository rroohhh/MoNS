...
namespace cpptoml {
    ...
    inline std::ostream & operator<<(std::ostream & os, const datetime & dt) {
        ...
        if(dt.microsecond > 0) { os << "." << setw(6) << dt.microsecond; }
    ...
    struct valid_value {
        const static bool value =
            ...
            std::is_same<T, bool>::value || std::is_same<T, datetime>::value;
    };

    template <class T,
              bool Valid = valid_value<typename std::decay<T>::type>::value ||
    ...
    inline std::shared_ptr<const value<T>> base::as() const {
        if(auto v =
               std::dynamic_pointer_cast<const value<T>>(shared_from_this()))
            return v;
        return nullptr;
    }
    ...
    public:
        ...
        std::vector<std::shared_ptr<array>> nested_array() const {
            ...
            std::transform(
                values_.begin(), values_.end(), result.begin(),
    ...
    private:
        ...
        void parse_key_value(std::string::iterator & it,
                             std::string::iterator & end, table * curr_table) {
    ...
    public:
         ...
         */
        void visit(const value<std::string> & v, bool = false) { write(v); }

        /**
         * Output an integer element of the TOML tree
         */
        void visit(const value<int64_t> & v, bool = false) { write(v); }

        /**
         * Output a double element of the TOML tree
         */
        void visit(const value<double> & v, bool = false) { write(v); }

        /**
         * Output a datetime element of the TOML tree
         */
        void visit(const value<datetime> & v, bool = false) { write(v); }

        /**
         * Output a boolean element of the TOML tree
         */
        void visit(const value<bool> & v, bool = false) { write(v); }
    ...
    protected:
         ...
         */
        void write(const value<int64_t> & v) { write(v.get()); }
         ...
         */
        void write(const value<datetime> & v) { write(v.get()); }
    ...
    private:
         ...
         */
        void indent() {
            for(std::size_t i = 1; i < path_.size(); ++i) write("\t");
        }
