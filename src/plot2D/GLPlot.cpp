...
int GLPlot::m_createWindow() {
	...
	sf::RenderWindow * window = new sf::RenderWindow(sf::VideoMode(800, 600), APPLICATION_NAME, sf::Style::Default, settings);
	...
	glBufferData(GL_ARRAY_BUFFER, 2 * m_count * sizeof(float), m_data, GL_STREAM_DRAW);
	glBindVertexArray(m_points.vao);

	m_shaderProgram = new ShaderProgram("#version 150\nuniform vec2 center;\nuniform vec2 scale;\nin vec2 vp;\nvoid main() {\n   gl_Position = vec4((vp.x + center.x) / scale.x, (vp.y - center.y) / scale.y, 0.0, 1.0);\n}\n", "#version 150\nout vec4 frag_colour;\nvoid main () {\n    frag_colour = vec4(0.0, 0.0, 0.0, 1.0);\n}");
...
int GLPlot::m_renderLoop(sf::RenderWindow * window) {
    sf::Event event;
	bool mouseDown = false, lines = false, scaleX = false, scaleY = false, axisNeedUpdate = true;
	...
	m_generateAxisData(screenSizeX, screenSizeY, centerX, centerY, scale[0], scale[1]);

	glBindBuffer(GL_ARRAY_BUFFER, m_axis.vbo);
	glBufferData(GL_ARRAY_BUFFER, 256 * sizeof(float), m_axisData, GL_STREAM_DRAW);
    ...
    while (window->isOpen()) {
		...
		if (axisNeedUpdate) {
			m_numberRenderer->setScreenDimension(screenSizeX, screenSizeY);
			m_generateAxisData(screenSizeX, screenSizeY, centerX - mouseDXScreen, centerY - mouseDYScreen, scale[0], scale[1]);
		...
		m_shaderProgram->uniform2f("center", centerX - mouseDXScreen, centerY - mouseDYScreen);
...
int GLPlot::m_generateAxisData(int screenWidth, int screenHeight, float centerX, float centerY, float scaleX, float scaleY) {
	centerX = -centerX;
	centerY = centerY;
...
inline float * GLPlot::m_pixelsToCoords(int x, int y, int screenWidth, int screenHeight) {
