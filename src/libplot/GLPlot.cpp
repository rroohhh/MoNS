...
int GLPlot::m_createWindow() {
    ...
    if(status != GLEW_OK) {
        log::debug("glewInit error: {}", glewGetErrorString(status));
        exit(-1);
    }
    ...
    log::debug("GLEW_VERSION : {}", glewGetString(GLEW_VERSION));
    log::debug("GLSL_VERSION : {}", glGetString(GL_SHADING_LANGUAGE_VERSION));

    if(!GLEW_VERSION_3_3) {
        log::debug(
            "Atleast version 3.3 of OpenGL is needed for this program to work");
        exit(-1);
    }

    sf::ContextSettings settings = window->getSettings();

    log::debug("DEPTH BITS: {}", settings.depthBits);
